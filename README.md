# JsonBuilder
JsonBuilder is a tool for converting .csv data to a structured JSON format. It is built on top of Pandas, and as such it requires the user to have  Pandas installed on their machine. A functional requirement when building this tool was to make it easy to run from a fully serialized and human readable state, which is why string based expressions are used in the interface.
...
Even in a simple scenario like this there are virtually endless ways of converting the .csv into JSON format, some more sensible than others of course. If you have an application that consumes JSON data in a certain format, and you receive data in a flat .csv format, then this tool together with a mapping allows you to easily convert the .csv data to the specific JSON format that can be consumed by your application. 
...
|-------------:|-----------|
|``"type"``          |(string) Can be ``"object"``, ``"array"``, or ``"primitive"``, defaults to ``"primitive"``|
|``"value_col"``     |(string) The header of the column containing the _value_ to be fetched|
|``"name_col"``      |(string) The header of the column containing the _name_ to be fetched |
|``"value"``         |(string) Can be used for setting a default value|
|``"name"``          |(string) Can be used for setting a default name|
|``"children"``      |(array) An array of all child nodes|
|``"multiple"``      |(bool) If set to ``true`` it means the JsonBuilder should write more than one instance of this node to the output, defaults to ``false``|
|``"group_by"``      |(string) Groups the DataFrame according to a Pandas group_by expression, for example: <br>``"df['some_column_name'].str[:3]"``.<br> Let ``s`` denote the raw string, the DataFrame is grouped according to ``df.group_by(eval(s))``. |
|``"filter"``        |(string) Applies a filter to the DataFrame by checking for truth values, for example: <br>``"df['currency1'] == 'EUR' & df['currency2'] == 'SEK'"``.<br>  Let ``s`` denote the raw string, the DataFrame is filtered according to ``df = df[eval(s)]``.|
|``"func"``          |(string) Applies a function to the value ``x`` that was generated by this node. , for example: <br>``"x if r['date']>"2020-04-03" else 0"``<br> ``x``, ``r``, and ``df`` are the variables at your disposal, the evaluated expression will be directly passed on as the value of this node.|
...
For simple operations it is typically more convenient to write an expression directly in the string, but for large/complex functions this functionality is essential.
